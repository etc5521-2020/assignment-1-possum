...
plate.sf <- st_polygon(x = list(as.matrix(rof %>% filter(plate == "KE") %>%  select(lon, lat)))) %>% 
  st_wrap_dateline()

plate.sf1 <- st_polygon(x = list(as.matrix(rof %>% filter(plate == "an") %>%  select(lon, lat)))) %>% 
  st_wrap_dateline()

plate.sf <- st_polygon(x = list(as.matrix(rof %>% filter(plate == "AP") %>%  select(lon, lat)))) %>% 
  st_wrap_dateline()
