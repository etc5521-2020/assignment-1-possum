...
world %>% 
  ...
  geom_point(data = volcano_erupted,
             aes(x = longitude, y = latitude,
                 ...
                 size = n)) +
  theme(legend.position = "bottom") +
  geom_polygon(data = filter(rof, plate == "KE"),
               aes(x = lon,
                   y = lat,
                   group = plate),
               fill = NA,
...
world <- world %>% 
  mutate(longfixed = case_when(long < 180 ~ 360 + long,
                               TRUE ~ long))
```
