...
p1 <- sulfur %>%
  ...
  geom_vline(xintercept = c(540, 575, 682), colour ="blue") +
  geom_vline(xintercept = c(520, 536, 574, 627, 550), colour ="green") +
...
p2 <- sulfur %>%
  ...
  scale_y_continuous(breaks = seq(0, 200, 25)) +
  geom_vline(xintercept = c(540, 575, 682), colour ="blue") +
  geom_vline(xintercept = c(683, 690, 653, 550), colour = "red") +
...
t1 <- tree_rings_long %>%
  ...
  geom_vline(xintercept = c(540, 575, 682), colour ="blue") +
  geom_vline(xintercept = c(520, 536, 574, 627, 550), colour ="green") +
  geom_vline(xintercept = c(683, 690, 653), colour = "red") +
  labs(title = "European Weather Index and Tree Ring Z-scores",
       x = "year",
       y = "Temp index/Z-score") +
  theme_linedraw()
```



```{r plot, fig.cap = "Time series of the sulfur events that have been recorded in Greenland and Antartica and the European weather index and Tree Ring Z-Score", fig.width = 12}
p2 + p1 + t1 + plot_layout(nrow = 3)
```

```{r violin-plot, fig.cap = "Figure displaying the distribution of eruptions with a VEI rating of 4 or greater"}
volcano_vei_ce <- volcano_eruptions_hemisphere_4 %>% 
  filter(year <= 705) %>%
  ggplot(aes(x = as.factor(vei),
             y = year,
             color = as.factor(vei))) +
  geom_violin() +
  facet_wrap(~hemisphere) +
  geom_point() +
  labs(title = "Distribution of VEI-4+ eruptions",
       x = "VEI Rating",
       y = "Year",
       color = "VEI") +
  scale_y_continuous(breaks = seq(500, 7010, 10)) +
  theme_linedraw()
volcano_vei_ce
```
...
```{r jma1, message=FALSE, echo=FALSE, eval = TRUE}
a5 <- rbind(a3, a4)
...
```{r jma2, message=FALSE, warning=FALSE, echo=FALSE, eval = TRUE}
 pal1 <- colorFactor(
...
National Geographic. (n.d.). Ring of Fire. Retrieved from https://www.nationalgeographic.org/encyclopedia/ring-fire/
...
Roger Bivand, Tim Keitt and Barry Rowlingson (2020). rgdal: Bindings
  ...
  1.5-15. https://CRAN.R-project.org/package=rgdal

Safety on volcanoes: risk zones around volcanoes. (2020). Retrieved 28 August 2020, from https://www.volcanodiscovery.com/volcanic_risk_zones.html#:~:text=4)%20Low%20Risk%20Zone&text=Typically%20you%20are%20more%20than,pyroclastic%20flows%20could%20be%20channeled.
...
Simon Garnier (2018). viridis: Default Color Maps from 'matplotlib'. R
  ...
  Tectonic Plate Boundaries. (2020). Retrieved 28 August 2020, from https://www.kaggle.com/cwthompson/tectonic-plate-boundaries
...
Tim Appelhans, Florian Detsch, Christoph Reudenbach and Stefan
  ...
  package version 2.9.0. https://CRAN.R-project.org/package=mapview
  
Wickham et al., (2019). Welcome to the tidyverse. Journal of Open
  Source Software, 4(43), 1686, https://doi.org/10.21105/joss.01686  
  
  Wikipedia. (n.d.). Volcanic Explosivity Index. Retrieved from https://en.wikipedia.org/wiki/Volcanic_Explosivity_Index
