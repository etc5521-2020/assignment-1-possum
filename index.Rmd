...
sulfur %>%
  ggplot(aes(x = neem,
             ...
             fill = year)) +
  geom_point() +
  scale_colour_gradient(low = "red", high = "blue", aesthetics = "fill")
...
volcano_eruptions_hemisphere_short <- volcano_eruptions_hemisphere %>%
  select(year, hemisphere)

rings_sulfur_volcano <- t_rings_ce %>%
  left_join(sulfur_yearly_means, by ="year") %>%
  left_join(volcano_eruptions_hemisphere_short, by = "year") %>%
  mutate(wdc_log = log10(mean_wdc),
         neem_log = log10(mean_neem)) %>%
  select(-mean_neem, -mean_wdc, -hemisphere)

rings_sulfur_volcano %>%
  filter(year <= 705) %>%
  ggscatmat(columns = 2:5)
```
...
percent_change %>%
  select(year, n_tree_pct_change, euro_temp_pct_change, mean_neem_pct_change, mean_wdc_pct_change) %>%
  pivot_longer(c("n_tree_pct_change", "euro_temp_pct_change", "mean_neem_pct_change", "mean_wdc_pct_change"),
               names_to = "index",
               values_to = "value") %>%
  ggplot(aes(x = year,
             y = value,
...
sulfur %>%
  arrange(year) %>%
  mutate(neem_pct_change = (neem/lag(neem ) - 1)*100,
         wdc_pct_change = (wdc/lag(wdc) - 1)*100) %>%
  ggplot(aes(x = year,
             y = neem_pct_change)) +
  geom_line()
