...
## Is there any relationship between tree rings and volcanic eruption events?
...
```{r tree-ring, out.width = '120%', fig.cap = "Tree ring z-score and temperature index"}
tree_ring_clean <- tree_rings %>% 
  filter(!is.na(year))

tree_ring_plot <- tree_ring_clean %>% 
  ggplot(aes(x = year, y = n_tree)) +
  geom_line(size = 0.5, alpha = 0.5) +
  geom_line(aes(x = year, y = europe_temp_index), color = 'red') +
...
tree_lm_1 <- lm(n_tree ~ europe_temp_index, data = tree_ring_clean)
tree_lm_2 <- lm(n_tree ~ europe_temp_index +eruption, data = tree_ring_clean)

tibble(model = c("lm1(no eruption)", "lm2(eruption)"), r_square = c(glance(tree_lm_1)[[2]],
glance(tree_lm_2)[[2]])) %>% kable(caption = "Compare two models, lm1 does not consider the effect of eruption") %>% 
  kable_styling(bootstrap_options = "striped", full_width = F, position = "left")
  
augment(tree_lm_2) %>% 
  ggplot(aes(x = n_tree, y = .resid)) +
  geom_point() +
...
tidyverse [@tidyverse], lubridate [@lubridate], broom [@broom], leaflet [@leaflet], ggmap [@ggmap], mapview[@mapview], viridis [@viridis], rgdal [@rgdal], kableExtra [@kableExtra], gridExtra [@gridExtra], readr [@readr], knitr [@knitr], sf [@sf], data.table [@data], ggthemes [@ggthemes], maps [@maps], ggridges [@ggridges], rvest [@rvest], plotly [@plotly]
