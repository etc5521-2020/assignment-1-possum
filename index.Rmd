...
There are 3 main types of Active Volcanoes (erupted before) namely Stratovolcano, Caldera and Shield. We decided to label the many others as others. 
...
This map zoomed into the Ring Of Fire with volcanoes running along the boundaries. As we can see, the volcanoes here are has more eruptions relatively speaking. 
...
```{r triangles-fun, fig.align="center", fig.height=12, fig.width=15}
...
Above shows plot with dots filled with number of eruptions in each country. and larger dot sizes represent the number of population in the vicinity. 
...
Theoretically, VEI is ranges from 0 to infinity. VEI scale is on a log. Therefore each interval of increase (1) in VEI indicate an eruption 10x more powerful. Looking back at the last 132 million years, there were only 40 eruptions recorded with VEI-8 magnitude and 10 eruptions of VEI-7 in last 10,000 years.
...
The density plots above shows that likelihood of a VEI 4 and above is very unlikely (all less than 10%). In fact, almost 98% of all volcanoes have less than VEI-3. 
...
eruptions_1812 %>% 
  ...
  coord_flip() +
  # facet by VEI
  facet_wrap(~vei, ncol = 1)

# filter top 3 volcanoes with highest frequency
volcano_frequent <- eruptions_1812 %>% 
  filter(!is.na(vei)) %>% 
  ungroup() %>% 
  group_by(volcano_name, vei) %>%
  mutate(frequency = n()) %>% 
...
volcano_frequent$lag_year <- as.integer(volcano_frequent$lag_year)

volcano_frequent$volcano <- as.factor(str_c(volcano_frequent$volcano_name, "-", volcano_frequent$vei))

volcano_frequent %>% 
  filter(vei == 4) %>% 
  ggplot() +
  geom_point(aes(x = lag_year,
                y = as.factor(volcano))) +
  scale_x_continuous(breaks = seq(from = 1812, to = 2020, by =1))
